...
exports.loungeTLDR = loungeTLDR;
const firebase_1 = require("../firebase");
const gemini_1 = require("../utils/gemini");
async function loungeTLDR(req, res) {
    try {
        ...
        if (!userData || !userData.lastSeenLoungeAt) {
            ...
            lastSeen = firebase_1.admin.firestore.Timestamp.fromDate(new Date(Date.now() - 500 * 24 * 60 * 60 * 1000));
        }
        else {
            lastSeen = userData.lastSeenLoungeAt;
        }
        // 2️⃣ Fetch lounge messages after lastSeen
        const messagesSnap = await firebase_1.db
    ...
    `;
        ...
        await firebase_1.db.collection("loungeSummaries").add({
            userId,
            summary,
            from: lastSeen,
            to: firebase_1.admin.firestore.FieldValue.serverTimestamp(),
        ...
        await firebase_1.db.collection("users").doc(userId).set({ lastSeenLoungeAt: firebase_1.admin.firestore.FieldValue.serverTimestamp() }, { merge: true });
